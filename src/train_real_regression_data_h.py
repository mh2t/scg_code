...
if __name__ == "__main__":
    ...
    train_y = train_y[:, 0].view(len(train_y), 1)
    test_y = test_y[:, 0].view(len(test_y), 1)
    test_Y = test_Y[:, 0].view(len(test_Y), 1)
    min_v = min_v[0]
    max_v = max_v[0]
    ...
    model = Model(
        ...
        p=P).to(device)
    model.load_state_dict(torch.load(f"{out_path}/student_model_h.pt"))
    ...
    for i in tqdm(range(NUM_EPOCHS), total=NUM_EPOCHS):
        ...
        if (i+1) % 100 == 0:
            ...
            tqdm.write("")
            torch.save(model.state_dict(), f"{out_path}/model_{i}_h.pt")

    model = model.to("cpu")
    torch.save(model.state_dict(), f"{out_path}/model_h.pt")

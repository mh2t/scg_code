...
if __name__ == "__main__":
    ...
    train_y = train_y[:, 2].view(len(train_y), 1)
    test_y = test_y[:, 2].view(len(test_y), 1)
    test_Y = test_Y[:, 2].view(len(test_Y), 1)
    min_v = min_v[2]
    max_v = max_v[2]
    ...
    model = Model(
        ...
        p=P).to(device)
    model.load_state_dict(torch.load(f"{out_path}/student_model_s.pt"))
    ...
    model = model.to("cpu")
    torch.save(model.state_dict(), f"{out_path}/model_s.pt")

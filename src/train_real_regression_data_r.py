...
if __name__ == "__main__":
    ...
    train_y = train_y[:, 1].view(len(train_y), 1)
    test_y = test_y[:, 1].view(len(test_y), 1)
    test_Y = test_Y[:, 1].view(len(test_Y), 1)
    min_v = min_v[1]
    max_v = max_v[1]
    ...
    for i in tqdm(range(NUM_EPOCHS), total=NUM_EPOCHS):
        ...
        if (i+1) % 100 == 0:
            ...
            tqdm.write(f"MAE of R = {test_losses[0]:.3f}")
            tqdm.write("")

    model = model.to("cpu")
    torch.save(model.state_dict(), f"{out_path}/model_r.pt")
